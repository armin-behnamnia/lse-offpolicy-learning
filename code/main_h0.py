    ...
    for x, y, action, delta, prop in reader.iter():
        ...
        if hyper_params.experiment.regularizers:
            if 'KL' in hyper_params.experiment.regularizers:
                loss += KLLoss(output, action, prop) * hyper_params.experiment.regularizers.KL
            if 'KL2' in hyper_params.experiment.regularizers:
                loss += KLLossRev(output, action, prop) * hyper_params.experiment.regularizers.KL2
            if 'SupKL' in hyper_params.experiment.regularizers:
                loss += SupKLLoss(output, action, delta, prop, hyper_params.experiment.regularizers.eps) * hyper_params.experiment.regularizers.SupKL
        loss.backward()
        optimizer.step()
        ...
        control_variate += torch.mean(output[range(action.size(0)), action] / prop).item()
    ...
    try:
        ...
        for exp in range(hyper_params.experiment.n_exp):
            ...
            for epoch in range(1, hyper_params['epochs'] + 1):
                ...
                metrics = train(model, criterion, optimizer, scheduler, train_reader, hyper_params, device=device)
                ...
                metrics = evaluate(model, criterion, val_reader, hyper_params, device=device)
                string2 = ""
                ...
                if metrics['Acc'] > best_acc:
                    ...
                    metrics = evaluate(model, criterion, test_reader, hyper_params, device=device)
                    string3 = ""
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--proportion', required=True, type=float, help='Proportion of data to be trained with')
    parser.add_argument('-d', '--device', required=True, type=str, help='Device')
    parser.add_argument('-t', '--tau', required=True, type=float, help='Softmax temparature')
    args = parser.parse_args()
    proportion = args.proportion
    tau = args.tau
    model = main(args.config, device=args.device, return_model=True, proportion=proportion, tau=tau)
    torch.save(model.state_dict(), f'models/h0_{proportion}_tau{tau}.pth')
