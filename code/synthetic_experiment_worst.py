...
def calculate_lambda_value(p, q, func_values, eps=1.0):
    significance = 0.1
    n = len(p)
    e_value = np.exp(1.0)
...
class Experiment:
    ...
    def run_experiments(self, method, correct_expected_value, q, p, func_values, epsilon):
        errors = []
        if self.adaptive_lambda and method == 'lse_adapt':
            calculator = getattr(self, f'calculate_lse_expected_value')
            # _lambda = 1 / len(q) ** (1 / (epsilon + 1))
            _lambda = calculate_lambda_value(p, q, func_values, eps=epsilon)
            # print(_lambda)
            # _lambda = 1.0
