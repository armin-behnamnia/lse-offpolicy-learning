    ...
    for x, y, action, delta, prop, labeled in reader.iter():
        ...
        if su > 0:
            if hyper_params.experiment.feedback == 'supervised':
                ...
                loss = criterion(output_labeled, action_labeled, delta_labeled, prop_labeled)
        ...
        if hyper_params.experiment.regularizers:
            ...
            if 'SupKL' in hyper_params.experiment.regularizers:
                if su > 0:
                    loss += SupKLLoss(output_labeled, action_labeled, delta_labeled, prop_labeled, hyper_params.experiment.regularizers.eps) * hyper_params.experiment.regularizers.SupKL
...
def main(config_path, device='cuda:0', return_model=False):
    ...
    try:
        ...
        for exp in range(hyper_params.experiment.n_exp):
            ...
            if 'lr_sch' in hyper_params and hyper_params['lr_sch'] == 'OneCycle':
                scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=hyper_params['lr'], epochs=hyper_params['epochs'], steps_per_epoch=len(train_reader))
            else:
            ...
            for epoch in range(1, hyper_params['epochs'] + 1):
                ...
                metrics = evaluate(model, criterion, val_reader, hyper_params, device, labeled=False)
                string2 = ""
                ...
                if metrics['Acc'] > best_acc:
                    ...
                    metrics = evaluate(model, criterion, test_reader, hyper_params, device, labeled=True)
                    string3 = ""
