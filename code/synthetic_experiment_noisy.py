    ...
    def run(self, noise_levels):
        ...
        if args.plot:
            for i, noise_level in enumerate(noise_levels):
                ...
                plt.grid()
                plt.legend()
                plt.savefig(f'plots/{args.type}/{"noisy" if args.noisy else "normal"}1/plot_{noise_level}.{FORMAT}', format=FORMAT)
                plt.figure(figsize=(14, 8))
                for k in errors.keys():
                ...
                plt.grid()
                plt.legend()
                plt.savefig(f'plots/{args.type}/{"noisy" if args.noisy else "normal"}2/plot_{noise_level}.{FORMAT}', format=FORMAT)
...
def plot_results(xaxis, yaxis):
    ...
    plt.grid()
    plt.legend()
    plt.savefig(f'plots/{args.type}/plot_{"noisy" if args.noisy else "normal"}_bad.{FORMAT}', format=FORMAT)
    ...
    plt.grid()
    plt.legend()
    plt.savefig(f'plots/{args.type}/plot_{"noisy" if args.noisy else "normal"}_good.{FORMAT}', format=FORMAT)
...
if args.multi_n:
    ...
    if args.type == 'gaussian':
        if args.noisy:
            ...
            alpha = 1.4
            noise_levels = np.linspace(-100, -101, num=2)
            print(f"ALPHA = {alpha}:")
            print('-' * 50)
            new_exp = Experiment(args.n, (1.0, 0.25), (0.5, 0.25), alpha, args.n_exp, 'gaussian', args.adaptive_lambda)
            mses = new_exp.run(noise_levels)
        ...
        if args.noisy:
            ...
            p = 2.5, 1.0
            q = 1.5, 1.0
            noise_levels = np.linspace(10000, 10001, num=2)
            set1 = []
